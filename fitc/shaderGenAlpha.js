function shaderGenAlpha(base){
	...
	this.parts['n'] = "c.rgb = cos(c.rgb+vec3(floor(uv*vec2(4.)), c.r*2.))*length(mod(c.rgb, vec3(1.0)));\n";
	...
	this.parts['r'] = "c.rg -= boxes(uv, 0.5, 2.0)*0.25;\n";
	this.parts['s'] = "c.rgb = cos((c.rbg+vec3(sin(uv.x*3. + t),cos(uv.x + t), .33))*5.)*.5+.5;\n";
